  ...
  try {
    ...
    if (!cityCode || String(cityCode).trim().length !== 3) {
      return res.status(400).json({
        success: false,
        message:
          "cityCode is required and must be a 3-letter IATA city code (e.g., PAR)",
      });
    }
    ...
    const data = await searchHotelOffers({
      ...
      includeOffers:
        String(includeOffers) === "1" ||
        String(includeOffers).toLowerCase() === "true",
...
router.post("/book", protect, createHotelBooking);
router.get("/bookings/:userId", protect, getUserHotelBookings);
router.delete("/bookings/:id", protect, deleteHotelBooking);
